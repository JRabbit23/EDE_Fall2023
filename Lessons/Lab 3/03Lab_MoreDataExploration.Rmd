...
> Answer: The data appeared as a dataframe, a list, and four matrixes.
...
```{r DataExercise-1a}
DurhamHighsF<-c(81,83,83,84,78,71,79,82,85,84)
DurhamLowsF<-c(55,56,58,60,61,63,63,64,64,63)
```
...
```{r DataExercise-1b}
DurhamHighsC<-((DurhamHighsF-32)*5/9)
DurhamLowsC<-((DurhamLowsF-32)*5/9)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
TempDF<-data.frame(DurhamHighsF,DurhamHighsC,DurhamLowsF,DurhamLowsC)
print(TempDF)
```
